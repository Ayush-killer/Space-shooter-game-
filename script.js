...
// 3. Start Game & Dynamic Timers
function startGame(n) {
    currentLvl = n; isPlaying = true;
    document.getElementById("menu-screen").classList.remove("active");
    
    // Timer Mastery: Tere bataye hue criteria ke hisaab se
    if(n <= 10) timer = 120;       // 2 min
    else if(n <= 20) timer = 240;  // 4 min
    else if(n <= 30) timer = 360;  // 6 min
    else if(n <= 40) timer = 480;  // 8 min
    else timer = 600;              // 10 min
...
// 5. Mobile Controls (Finger Offset)
window.addEventListener("touchmove", (e) => {
    if(!isPlaying) return;
    player.x = e.touches[0].clientX;
    player.y = e.touches[0].clientY - 80; // Finger ke upar ship
});
...
function update() {
    ...
    if(Date.now() % 150 < 20) {
        ...
        if(player.triple) { bullets.push({x: player.x, y: player.y-20, vx: -2.5}, {x: player.x, y: player.y-20, vx: 2.5}); }
    }

    // --- ENEMY SCALING LOGIC ---
    // Level ke hisaab se dushman badhenge
    let spawnChance = 0.015 + (currentLvl * 0.0012); 
    if(Math.random() < spawnChance) {
        let eSpeed = 2.5 + (currentLvl * 0.08);
        enemies.push({x: Math.random()*canvas.width, y: -50, s: eSpeed});
    }
    ...
    enemies.forEach((e, i) => {
        ...
        bullets.forEach((b, bi) => {
            if(Math.hypot(e.x-b.x, e.y-b.y) < 30) {
                ...
                score += 10 * combo; combo++; // Combo System
    ...
    // Boss AI
    if(boss) {
        boss.y = Math.min(boss.y + 1, 100); boss.x += boss.dir * (2 + currentLvl/10);
